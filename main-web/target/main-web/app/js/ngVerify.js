...
(function () {
    var m = angular.module('ngVerify', ['pascalprecht.translate']);
    //定义一个需要国际化的数组
    var vi = "global.messages.validate";
    var arrFormatter = new Array(vi + ".notNull",vi + ".choose",vi + ".item",vi + ".select",vi + ".inputNum",vi + ".least",
    		vi + ".more",vi + ".char",vi + ".number",vi + ".integer",vi + ".plus",vi + ".plusAndZero",vi + ".letter",vi + ".letterAndNum",vi + ".userEmail",
    		vi + ".telPhone",vi + ".idCard",vi + ".Chinese",vi + ".url",vi + ".date",vi + ".pleaseInput",vi + ".format",vi + ".inputRegular",vi + ".notSame",vi + ".chars");
    //定义一个存放翻译后的参数的数组
    var arrParam = new Array();
    
    //初始化国际化
    function formatterParams ($translate){
    	for (var i =0;i<arrFormatter.length;i++ ){
    		arrParam.push($translate.instant(arrFormatter[i]));
    	}
    }
    ...
    m.directive('ngVerify', ['ngVerify','$translate',function (ngVerify,$translate) {
    	formatterParams($translate);
        return {
            require: "?^verifyScope",
            scope: false,
            link: function (scope, iElm, iAttrs, pCtrl) {
                ...
                setTimeout(function () {
                    ...
                    var OPTS = formatOpt(iAttrs.ngVerify, iElm);
                     //自定义的错误信息进行国际化
                	if (OPTS.errmsg != undefined) {
                		OPTS.errmsg = $translate.instant(OPTS.errmsg);
                	}
    ...
    function Init(iElm) {
        ...
        // 默认配置
        var DEFAULT = {
            message: 'global.messages.validate.notNull', //默认错误消息
            required: true, //默认都进行非空校验
            asterisk: true, //默认必填字段加红色*
            option: 0, //下拉菜单校验
            ...
            tipStyle: $scope.ngVerify.tipStyle >= 0 ? $scope.ngVerify.tipStyle : 3 //tip提示样式, 0禁用 1 右上浮动  2 左下占位 3 右边对齐
        };
        ...
        if (OPTS.control || iAttrs.type == 'submit') {
         ...
         // 创建tip容器
            ...
            //将必填字段补充红色*
            if(OPTS.required && OPTS.asterisk){
        	   errtip = angular.element(
                  '<span class="span_nec">*</span><div class="verifyTips">' +
                  '<p class="tipStyle-' + OPTS.tipStyle + '">' +
                  '<span class="tipMsg"></span>' +
                  '<i></i>' +
                  '</p>' +
    	   ...
    	   //将必填字段补充红色*,日期类型样式
    	   if(OPTS.required && iAttrs.type === 'dates' && OPTS.asterisk){
    		   errtip = angular.element(
    				   '<span class="span_nec_dates">*</span><div class="verifyTips">' +
    				   '<p class="tipStyle-' + OPTS.tipStyle + '">' +
    				   '<span class="tipMsg"></span>' +
    				   '<i></i>' +
    				   '</p>' +
    ...
    function tipMsg(iElm, isShow) {
      ...
      //  errtip.message.attr('data-translate', message);
        ...
        if (OPTS.tipStyle == 3) {
            ...
            var tipWidth;
            if (iElm.attr('type') === 'radio' || iElm.attr('type') === 'checkbox') {
                // 此类型元素dom无高度,取外层容器高度和宽度
                tipHeight = iElm.parent()[0].offsetHeight;
                tipWidth = iElm.parent()[0].offsetWidth;
            } else {
                tipHeight = iElm[0].offsetHeight;
                tipWidth = iElm[0].offsetWidth + 160;
            }
            if (iElm.attr('type') === 'dates') {
            	errtip.tip.css('left', tipWidth-50 + 'px');
            }else if (iElm[0].localName == 'select'){ 
            	errtip.tip.css('left', tipWidth-5 + 'px');
        	}else{
        		errtip.tip.css('left', tipWidth-5 + 'px');
        	}
        }
    ...
    function makeError(iElm, draw) {
        ...
        if (iElm[0].type == 'checkbox' || iElm[0].type == 'radio') {
            ...
            //调用此方法是第一次失焦即显示验证的信息,此方法是smile哥加的
            tipMsg(iElm,true);
        }
    ...
    function ISVALID(iElm) {
        ...
        if (iAttrs.type == 'checkbox' || iAttrs.type == 'radio') {
           ...
           // OPTS.message = '至少选择' + OPTS.least + '项';
            OPTS.message = arrParam[1] + OPTS.least + arrParam[2];
            return false;
        }

        // select元素验证
        if (iElm[0].localName == 'select') {
            if (iElm[0].selectedIndex === OPTS.option && iElm[0].value=="") {
             //   OPTS.message = '请选择';
                OPTS.message = arrParam[3];
                return false;
            } else {
                return true;
            }
        ...
        if (OPTS.required && val == '') {
            ...
            if (iAttrs.type == 'number') {
              //  OPTS.message = '需输入数字'; //tip1
            	OPTS.message =arrParam[4];
            } else {
            	OPTS.message =arrParam[0];//tip2
            }
            return false;
        ...
        if (val.length < OPTS.min) {
            OPTS.message = arrParam[5] + OPTS.min + arrParam[7];
            return false;
        }
        if (val.length > OPTS.max) {
            OPTS.message = arrParam[6] + OPTS.max + arrParam[7];
            return false;
        }
        ...
        // 获取type类型，多重验证用逗号分隔;
        var type =  iAttrs.type.split(",");
        var groupLen = type.length;
        for ( var i = 0; i < groupLen; i++) {
            var validateType = type[i].trim();
      /*      //自定义方法
            if(validateType.indexOf("function:") > -1) {
                var fs = validateType.split(':');
                var result = eval([fs[1]]+ '()');
            ...
            switch (validateType) {
                ...
                case 'plus':
                    pat = /^(\+)?\d+($|\.\d+$)/; //正数（包含小数）
                    pat.name = arrParam[10];
                    break;
                case 'plusInteger':
                    pat = /^\+?[0-9][0-9]*$/; //正整数和0
                    pat.name = arrParam[11];
                    break;
                case 'letter':
                    pat = /^[A-Za-z]+$/; // 字母
                    pat.name = arrParam[12];
                    break;
                case 'variable':
                    pat = /^[A-Za-z0-9]+$/; // 数字和字母
                    pat.name = arrParam[13];
                    break;
                case 'email':
                    pat = /^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/;
                    pat.name = arrParam[14];
                    break;
                case 'phone':
                    pat = /^(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
                    pat.name = arrParam[15];
                    break;
                case 'IDCard':
                    pat = /^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$/;
                    pat.name = arrParam[16];
                    break;
                case 'cn':
                    pat = /^[\u4e00-\u9fa5]+$/; //中文
                    pat.name = arrParam[17];
                    break;
                case 'url':
                    pat = /(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?/;
                    pat.name = arrParam[18];
                    break;
                case 'char':
                    pat = /^[A-Za-z_]+$/; // 字母+下划线
                    pat.name = arrParam[24];
                    break;
                case 'dates' || 'date':
        ...
        if (pat) {
            // 验证正则
            if (val.match(pat)==null && pat.name !="") {
                //OPTS.message = "请输入"+pat.name + '的格式';
            	OPTS.message = arrParam[20]+pat.name + arrParam[21];
                return false;
            }else if (val.match(pat)==null && pat.name == "") {
              //  OPTS.message = "请输入正则表达式的格式";
                OPTS.message = arrParam[22];
                return false;
            } else {
                return true;
            }
