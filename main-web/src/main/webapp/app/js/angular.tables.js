                ...
                for(var index = 0;index < titles.length;index++){
    				...
    				if(dynamicTitles != undefined){
	                    ...
	                    if (type === 'date'){
	                    	...
	                    	content += '<div ng-repeat="detail in item. '+ details +'"><span ng-if="detail.type =='+ "1" +' " data-toggle="popover" title="{{detail.startDate}}" style="margin-right:15px">'+ 
	                    	'{{detail.startDate | date:"yyyy-MM-dd"' + '}}</span> <span ng-if="detail.endDate != detail.startDate && detail.type ==1" style="margin-left:-15px">&mdash; </span> <span ng-if="detail.endDate != detail.startDate && detail.type ==1">{{detail.endDate | date:"yyyy-MM-dd"}}<span style="margin-right:15px">&nbsp;&nbsp;&nbsp;</span></span></div>';	
	                    }else if (type == 'supplement_date'){
	                    	var details = titles[index]['filed'];  
	                    	content += '<div ng-repeat="detail in item. '+ details +'"><span ng-if="detail.type =='+ "2" +' " data-toggle="popover" title="{{detail.startDate}}" style="margin-right:15px">'+ 
	                    	'{{detail.startDate | date:"yyyy-MM-dd"' + '}}</span> <span ng-if="detail.endDate != detail.startDate && detail.type ==2" style="margin-left:-15px">&mdash; </span> <span ng-if="detail.endDate != detail.startDate && detail.type ==2">{{detail.endDate | date:"yyyy-MM-dd"}}<span style="margin-right:15px">&nbsp;&nbsp;&nbsp;<span></span></div>';	                   	
	                    }else if (type === 'dateTime'){
	                    	if(format != undefined){
